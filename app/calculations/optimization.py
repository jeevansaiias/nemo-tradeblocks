...
class OptimizationCalculator:
    ...
    def optimize_portfolio(
        self, portfolio: Portfolio, request: OptimizationRequest
    ) -> OptimizationResult:
        ...
        try:
            ...
            for strategy in strategies:
                strategy_trades = [
                    trade for trade in portfolio.trades if trade.strategy == strategy
                ]
            ...
            optimal_weights = {
                strategy: 1.0 / num_strategies for strategy in strategy_returns.keys()
            }

            # Calculate portfolio metrics
            expected_return = sum(
                weight * strategy_returns[strategy] for strategy, weight in optimal_weights.items()
            )
            volatility = np.sqrt(
                sum(
                    (weight * strategy_volatility[strategy]) ** 2
