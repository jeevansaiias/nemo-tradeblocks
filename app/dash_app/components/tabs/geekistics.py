...
def create_geekistics_tab():
    ...
    return dmc.Stack(
        children=[
            ...
            ], justify="space-between", align="center", mb="lg"),
        ...
        children=[
            dmc.Group(
                children=[
                    ...
                    DashIconify(
                        icon=icon or "tabler:chart-line",
                        width=32,
                        height=32,
                        color=color
                    ) if icon else None
...
def create_comprehensive_stats(portfolio_stats, strategy_stats, trades_data, selected_strategies=None, advanced_stats=None):
    ...
    if selected_strategies:
        filter_badge = dmc.Group([
            ...
            dmc.Text(f"({', '.join(selected_strategies[:3])}{'...' if len(selected_strategies) > 3 else ''})",
                    size="sm", c="dimmed")
        ], gap="xs")
    ...
    return dmc.Stack([
        ...
        dmc.Stack([
            ...
            ], p="md", withBorder=True, radius="md", mb="sm"),

            dmc.SimpleGrid([
                create_stat_card("Number of Trades", f"{total_trades:,}", color="blue"),
                create_stat_card("Starting Capital", f"${initial_capital:,.0f}", color="blue"),
                create_stat_card("Avg Return on Margin", f"↗ {avg_return_on_margin:.2f}%", color="green"),
                create_stat_card("Std Dev of RoM", f"{std_dev_rom:.2f}%", color="orange"),
                create_stat_card("Best Trade", f"↗ {best_trade_pct:.2f}%", color="green"),
                create_stat_card("Worst Trade", f"↘ {worst_trade_pct:.2f}%", color="red"),
            ], cols=3, spacing="sm")
        ], gap="xs"),
        ...
        dmc.Stack([
            dmc.Group([
                ...
                dmc.Badge("Position-Size Dependent", variant="light", color="blue", size="sm"),
            ], justify="space-between", align="center", mb="sm"),

            dmc.SimpleGrid([
                create_stat_card("Total P/L", f"${portfolio_stats.get('total_pl', 0):,.0f}", color="green" if portfolio_stats.get('total_pl', 0) > 0 else "red"),
                create_stat_card("CAGR", f"{cagr:.2f}%", color="green" if cagr > 0 else "red"),
                create_stat_card("Avg RoM", f"{avg_return_on_margin:.2f}%", color="blue"),
                create_stat_card("Win Rate", f"{portfolio_stats.get('win_rate', 0)*100:.2f}%", color="green"),
                create_stat_card("Loss Rate", f"{(1-portfolio_stats.get('win_rate', 0))*100:.2f}%", color="red"),
            ], cols=5, spacing="sm")
        ], gap="xs"),
        ...
        dmc.Stack([
            dmc.Group([
                ...
                dmc.Badge("Position-Size Dependent", variant="light", color="orange", size="sm"),
            ], justify="space-between", align="center", mb="sm"),

            dmc.SimpleGrid([
                create_stat_card("Max Drawdown", f"{abs(portfolio_stats.get('max_drawdown', 0)):.2f}%", color="red"),
        ...
        dmc.Stack([
            dmc.Group([
                ...
                dmc.Badge("Position-Size Independent", variant="light", color="purple", size="sm"),
            ], justify="space-between", align="center", mb="sm"),
            ...
            dmc.SimpleGrid([
                ...
                create_stat_card("Kelly %", f"{kelly_criterion:.2f}%", color="orange" if kelly_criterion >= 0 else "red"),
            ], cols=5, spacing="sm")
        ], gap="xs"),
...
def create_stat_card(title, value, subtitle=None, color="blue"):
    ...
    return dmc.Paper([
        dmc.Stack([
            ...
            dmc.Text(value, size="xl", fw=700, c=color, ta="center", style={"lineHeight": 1.1}),
            dmc.Text(subtitle, size="xs", c="dimmed", ta="center", style={"lineHeight": 1.2}) if subtitle else None,
        ], gap="xs", justify="center", align="center", style={"height": "100%"})
    ], p="md", withBorder=True, radius="md", style={"height": "100px", "display": "flex", "alignItems": "center", "justifyContent": "center"})
            ...
            dmc.Table([
                ...
                dmc.TableTbody([
                    dmc.TableTr([
                        ...
                        dmc.TableTd(f"${stats['total_pl']:,.0f}", c="green" if stats['total_pl'] > 0 else "red", fw=500),
