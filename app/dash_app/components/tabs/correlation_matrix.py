...
def create_correlation_matrix_tab():
    ...
    return dmc.Stack(
        children=[
            ...
            dmc.Alert(
                children=[
                    dmc.Stack([
                        ...
                        dmc.List([
                            dmc.ListItem(dmc.Text("🟢 +1.0: Perfect positive correlation (strategies always move together)", size="xs")),
                            dmc.ListItem(dmc.Text("🔵 0.0: No correlation (strategies move independently)", size="xs")),
                            dmc.ListItem(dmc.Text("🔴 -1.0: Perfect negative correlation (strategies move opposite)", size="xs")),
                        ], size="xs", spacing="xs"),
                        dmc.Divider(),
                        dmc.Group([
                            dmc.Badge("Pearson", color="blue", variant="light", size="xs"),
                            dmc.Text("Linear relationships (normal)", size="xs", c="dimmed"),
                            dmc.Badge("Spearman/Kendall", color="orange", variant="light", size="xs"),
                            dmc.Text("Rank-based (non-linear patterns)", size="xs", c="dimmed")
                        ], gap="xs", align="center")
                    ], gap="xs")
                ],
                color="blue",
            ...
            dmc.Paper(
                children=[
                    html.Div(id="correlation-analytics-content",
                            children=[
                                dmc.Text("Load portfolio data to see correlation analysis",
    ...
    return dmc.Group([
        ...
        dmc.Group([
            ...
            dmc.Tooltip(
                ...
                children=[
                    DashIconify(icon="tabler:info-circle", width=20, color="gray")
                ],
                multiline=True
            )
        ], gap="xs", align="center"),
        ...
        children=[
            dmc.Stack([
                ...
                html.Div(
                    ...
                    children=[
                        dmc.Text("Load portfolio data to see correlation analytics",
                                size="sm", c="dimmed", ta="center", py="xl")
                    ]
                )
            ], gap="md")
        ],
        ...
        children=[
            dmc.Stack([
                ...
                dmc.Tabs(
                    ...
                    children=[
                        dmc.TabsList([
                            dmc.TabsTab("Correlation Matrix", value="matrix",
                                       leftSection=DashIconify(icon="tabler:table", width=16)),
                            dmc.TabsTab("Strategy Pairs", value="pairs",
                                       leftSection=DashIconify(icon="tabler:link", width=16)),
                            dmc.TabsTab("Risk Insights", value="insights",
                                       leftSection=DashIconify(icon="tabler:bulb", width=16)),
                            dmc.TabsTab("Export Data", value="export",
                                       leftSection=DashIconify(icon="tabler:download", width=16))
                        ]),

                        # Tab Panels
                        dmc.TabsPanel(
                            children=[create_correlation_table()],
                            value="matrix"
                        ),
                        dmc.TabsPanel(
                            children=[create_strategy_pairs_analysis()],
                            value="pairs"
                        ),
                        dmc.TabsPanel(
                            children=[create_risk_insights_panel()],
                            value="insights"
                        ),
                        dmc.TabsPanel(
                            children=[create_export_panel()],
                            value="export"
                        )
                    ]
                )
...
def create_correlation_table():
    ...
    return dmc.Container([
        ...
        html.Div(
            ...
            children=[
                dmc.Text("Load portfolio data to see correlation matrix",
                        size="sm", c="dimmed", ta="center", py="xl")
            ]
        )
    ], fluid=True)
...
def create_strategy_pairs_analysis():
    ...
    return dmc.Container([
        ...
        html.Div(
            ...
            children=[
                dmc.Text("Load portfolio data to analyze strategy pairs",
                        size="sm", c="dimmed", ta="center", py="xl")
            ]
        )
    ], fluid=True)
        ...
        dmc.Stack([
            dmc.Alert(
                children=[
                    ...
                    html.Ul([
                        html.Li("Look for strategies with correlation < 0.3 for better diversification"),
                        html.Li("Avoid overweighting highly correlated strategies (>0.7)"),
                        html.Li("Negative correlations can provide hedge benefits"),
                        html.Li("Monitor correlation stability over different time periods")
                    ])
                ],
                color="blue",
                variant="light"
...
def create_sample_heatmap():
    ...
    strategies = ["30DTE Fly", "Iron Condor", "Put Credit Spread", "Call Credit Spread", "Jade Lizard"]
