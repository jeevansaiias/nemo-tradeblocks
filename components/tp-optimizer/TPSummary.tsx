...
export function TPSummary() {
  const { data, baseline, best, results, candidates, objective } = useTPOptimizerStore();
  ...
  const summaryData = useMemo(() => {
    ...
    const pfImprovement = best.profitFactor - baseline.profitFactor;
  ...
  const generateNarrative = () => {
    ...
    const tpLevel = best.tpPct;
    const improvement = summaryData.improvement;
    const winRateChange = summaryData.winRateImprovement;
    const pfChange = summaryData.pfImprovement;

    if (improvement <= 0) {
      return `Auto-TP analysis shows that your baseline (hold-to-expiry) strategy outperforms take-profit exits. Keeping your current rules may be better than implementing a ${tpLevel}% TP.`;
    }

    return `Auto-TP at ${tpLevel}% improved total P&L by ${improvement.toFixed(1)}% over baseline (win rate ${winRateChange >= 0 ? '+' : ''}${winRateChange.toFixed(1)}pp, PF ${baseline.profitFactor.toFixed(2)} → ${best.profitFactor.toFixed(2)}).`;
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your current strategy performance</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <tbody>
                {summaryData.sortedResults.map((result, index) => (
                  <tr key={result.tpPct} className={`border-b hover:bg-muted/50 ${result.tpPct === best.tpPct ? 'bg-green-50 dark:bg-green-950' : ''}`}>
                    ...
                    <td className="p-2 font-medium">{result.tpPct}%</td>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                Based on this analysis, consider implementing a {best.tpPct}% take-profit rule for your {data.length} trades. 
