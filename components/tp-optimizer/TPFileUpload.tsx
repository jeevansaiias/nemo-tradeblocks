...
export function TPFileUpload({ onDataLoaded }: TPFileUploadProps) {
  ...
  const parseCSVData = useCallback((csvText: string): TradeRecord[] => {
    ...
    for (const row of rows) {
      try {
        // Map headers to TradeRecord fields
        const strategy = String(row["Strategy"] || row["strategy"] || "Unknown");
        
        // Handle dates
        const entryDate = parseDate(String(row["Entry Date"] || row["entry_date"] || row["Date"]));
        const exitDate = parseDate(String(row["Exit Date"] || row["exit_date"] || row["Date"]));
        
        // Handle percentages (remove % sign if present)
        const maxProfitPct = parsePercentage(row["Max Profit %"] || row["max_profit_pct"] || row["Max Profit"]);
        const maxLossPct = parsePercentage(row["Max Loss %"] || row["max_loss_pct"] || row["Max Loss"]);
        const resultPct = parsePercentage(row["Result %"] || row["result_pct"] || row["Result"]);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-muted/50 rounded-lg p-4">
        ...
        <div className="space-y-1 text-sm text-muted-foreground">
          <p><strong>Required columns:</strong> Strategy, Entry Date, Exit Date, Max Profit %, Max Loss %, Result %</p>
          <p><strong>Date format:</strong> YYYY-MM-DD, MM/DD/YYYY, or any standard format</p>
          <p><strong>Percentage format:</strong> Numbers (with or without % sign)</p>
          <div className="mt-2">
            <Badge variant="outline" className="text-xs">
              Example: Strategy=&quot;Iron Condor&quot;, Max Profit %=&quot;15.5&quot;, Result %=&quot;12.3&quot;
            </Badge>
          </div>
        </div>
      </div>
