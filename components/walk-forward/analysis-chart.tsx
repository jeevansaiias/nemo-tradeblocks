...
export function WalkForwardAnalysisChart({
  ...
  const timeline = useMemo(() => {
    ...
    const inSampleTrace: Data = {
      ...
      mode: "lines+markers",
      name: "In-Sample",
      x: periods.map((period) => midpoint(period.inSampleStart, period.inSampleEnd)),
      y: periods.map((period) => Number(period.targetMetricInSample.toFixed(3))),
    ...
    const outSampleTrace: Data = {
      ...
      mode: "lines+markers",
      name: "Out-of-Sample",
      x: periods.map((period) => midpoint(period.outOfSampleStart, period.outOfSampleEnd)),
      y: periods.map((period) => Number(period.targetMetricOutOfSample.toFixed(3))),
  ...
  const parameterEvolution = useMemo(() => {
    const parameterKeys = Array.from(
      new Set(periods.flatMap((period) => Object.keys(period.optimalParameters)))
    )
